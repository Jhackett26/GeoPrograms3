public class B2_TeachMeNestedLoops extends World
{
   ...
   {  
      ...
      for(int row=0; row<150; row=row+1)
      {    
         // This inner for loop begins on line 28 and ends on line 32
         for(int col=0; col<255; col=col+1)
         {
            plane.teleport(col, row);
            plane.setPixelColor(0,col,col); // Fills a pixel the red, green, blue (RGB) values you set
         }        	
      }
      //for(int row=0; row<800; row=row+10)
      {
         // This inner for loop begins on line 28 and ends on line 32
         //for(int col=500; col<1000; col=col+1)
         {
            //plane.teleport(col, row);
            //plane.setPixelColor(200,(int)col/4,(int)row/4); // Fills a pixel the red, green, blue (RGB) values you set
         }
      }
